...
class CropBackOfCardFragment: BaseCropBackOfFragment() {
   ...
   private fun handleError(reason :Reason){
       when(reason){
           is ApiError -> {
               android.util.Log.e("CropBackFragment", "ApiError: statusCode=${reason.statusCode}, message=${reason.message}")
